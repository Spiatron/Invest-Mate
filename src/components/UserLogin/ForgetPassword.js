...
const ForgetPassword = () => {
    ...
    // Validator for userID (alphanumeric, uppercase)
    ...
    // Handle userID submission (send OTP)
    const handleUserIDSubmit = async (values) => {
        setUserID(values.userID);

        const jsonBody = { userZID: values.userID };
    ...
    const handlePasswordReset = async () => {
        ...
        const jsonBody = { userZID: userID, newPassword: password, OTP: otp };
    ...
    return (
        <div style={{ height: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center', backgroundColor: '#f0f2f5' }}>
            <div style={{ backgroundColor: '#fff', padding: '40px', borderRadius: '10px', boxShadow: '0 4px 10px rgba(0, 0, 0, 0.1)', width: '100%', maxWidth: '400px' }}>
                ...
                {!isOTPSent ? (
                    <Form onFinish={handleUserIDSubmit} layout="vertical">
                        <Form.Item
                            name="userID"
                            rules={[
                                { required: true, message: 'Please input your User ID!' },
                                { validator: validateUserID }  // Custom validation for User ID
                            ]}
                        >
                            <Input
                                prefix={<UserOutlined />}
